...
|---|---|
| `app/assets/css/theme.css` | Every colour, size, radius and shadow in the project. The only file with a colour in it. |
| Nuxt UI (`U*` components) — do not rebuild them | Primitives — button, card, input. Small, reused everywhere. |
| `app/components/` | Everything built out of the primitives. |
| `app/pages/` | The screens. `index.astro` lists them all. |
| `app/fixtures/` | Fake data, typed. Never write data into a screen. |
